...
import type { Booking, Court, SlotInfo } from '../types';
    ...
    <div className="min-h-screen bg-slate-50/50 pb-16">
      <div className="bg-gradient-to-r from-emerald-900 via-teal-900 to-slate-900 text-white border-b border-emerald-800/30 shadow-lg">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-10 pb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-bold uppercase tracking-wider mb-3 backdrop-blur-sm border border-emerald-500/30">
            ...
            <div className="flex items-center bg-emerald-950/60 p-1.5 rounded-2xl border border-emerald-700/40 backdrop-blur-md self-start md:self-auto">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all duration-200 cursor-pointer min-h-[44px] ${
                  viewMode === 'day'
                    ? 'bg-emerald-500 text-white shadow-md shadow-emerald-600/30 scale-102'
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
              {courts.map((court) => (
                <button
                  ...
                  className={`shrink-0 text-xs sm:text-sm font-extrabold px-5 py-3 rounded-xl border transition-all duration-200 cursor-pointer min-h-[44px] ${
          ...
          {!rescheduling && (
            <div className="flex items-center gap-1.5 self-start md:self-auto bg-white p-1.5 rounded-xl border border-slate-200 shadow-xs">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider px-2">Duration:</span>
              {DURATIONS.map((d) => (
                <button
                  key={d}
                  onClick={() => setDuration(d)}
                  className={`px-3.5 py-2 rounded-lg text-xs font-black transition-all duration-200 cursor-pointer min-h-[36px] ${
                    duration === d
