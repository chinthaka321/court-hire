    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        <button
          onClick={prevWeek}
          disabled={!canGoPrev}
          className="w-7 h-7 flex items-center justify-center rounded-full text-on-surface-muted hover:bg-[#f0f0f0] disabled:opacity-25 disabled:cursor-not-allowed transition-colors"
        >
            ...
            <button
              ...
              className={`flex flex-col items-center justify-center rounded-xl py-2 transition-all ${
                isSelected
                  ...
                  : disabled
                  ? 'opacity-25 cursor-not-allowed text-on-surface-muted'
                  : 'hover:bg-[#f0f7f3] text-on-surface'
              }`}
            >
              <span className={`text-[10px] font-medium leading-none mb-1 ${isSelected ? 'text-white/70' : 'text-on-surface-muted'}`}>
                {format(day, 'EEE')}
              </span>
